 ...
 * 传入参数：saccount（学生账号）mdaccount（盲导账号）
 * 返回参数：status：true   message：success
 */
public class AddMangdaoServlet extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            if(rs.next()){
                sql="UPDATE association SET mdaccount=? WHERE saccount=?";
                pstmt = conn.prepareStatement(sql);
                pstmt.setString(1, mdaccount);
