...
public class KtbgMangdaoListServlet extends HttpServlet {
    public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            conn = DB.getConn();
            String sql = "select association.saccount,student.sname, ktbg.titleeng, ktbg.titlechi, ktbg.titletype, ktbg.titlereq, ktbg.pass, ktbg.comment, ktbg.grade " +
                    "from association, ktbg, student " +
                    "where association.mdaccount=? and association.saccount=ktbg.saccount and ktbg.saccount=student.sid";
            ...
            if (rs.next()){
                do{
                    mdmess.add(Json.createObjectBuilder()
                            ...
                            .add("teacher_pass", rs.getString(7) == null ? "" : rs.getString(7))
                            .add("teacher_comment", rs.getString(8) == null ? "" : rs.getString(8))
                            .add("grade", rs.getString(9) == null ? "" : rs.getString(9)));
                }while(rs.next());
